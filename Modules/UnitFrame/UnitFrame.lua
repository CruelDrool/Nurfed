...
local function HealthBar_OnUpdate(frame, e)
    ...
    -- if UnitExists(unit) then
		--if not frame.pauseUpdates then
			...
			local maxValue = addon:pcall(function() return UnitHealthMax(unit) end, 0)
...
if C_CurveUtil then
	...
	-- cooldownColorCurve:AddPoint(3, CreateColor(1, 0.5, 0, 1))
	cooldownColorCurve:AddPoint(5.4, CreateColor(1, 1, 0.1, 1))
	cooldownColorCurve:AddPoint(59.4, CreateColor(1, 1, 1, 1))
	cooldownColorCurve:AddPoint(3569.9, CreateColor(0.7, 0.7, 0.7, 1))
end

local function SetAuraTextScale(size, aura)
	if OmniCC then return end

	local sizeRatio = Round(min(size)) / 36
	local scaleRatio = aura:GetEffectiveScale() / UIParent:GetEffectiveScale()

	local textScale = sizeRatio * scaleRatio

	if textScale <= 0.45 then
		aura.cooldown:SetHideCountdownNumbers(true)
	else
		aura.cooldown:SetHideCountdownNumbers(false)
		-- aura.cooldown:GetCountdownFontString():SetScale(textScale)
		aura.cooldown:SetScale(textScale)
	end
end
		...
		if auraData then
			...
			if addon.WOW_PROJECT_ID == addon.WOW_PROJECT_ID_MAINLINE then
				...
				if duration then
					...
					local cooldownColor = duration:EvaluateRemainingDuration(cooldownColorCurve)
				...
				if PLAYER_UNITS[auraData.sourceUnit] then
					size = largeSize
					auraFrameHeight = largeSize
				end

				if auraData.applications > 1 then
					aura.count:SetText(auraData.applications)
					aura.count:Show()
				else
					aura.count:Hide()
				end
				...
				if auraData.duration > 0  then
					aura.cooldown:Show()
					CooldownFrame_Set(aura.cooldown, auraData.expirationTime - auraData.duration, auraData.duration, true, true, auraData.timeMod)
				else
					aura.cooldown:Hide()
				end

				aura.stealable:SetAlpha((not playerIsTarget and auraData.isStealable) and 1 or 0)

				if auraData.dispelName then
					colorInfo = DebuffTypeColor[auraData.dispelName]
				else
...
--[[
	...
	local debuffFilterPlayerOnly = "INCLUDE_NAME_PLATE_ONLY|PLAYER"
	local debuffFilterAll = "INCLUDE_NAME_PLATE_ONLY"
